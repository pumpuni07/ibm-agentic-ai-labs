...
GPT-4o-mini chats through permission-aware MCP tools: every tool call
is risk-assessed (low/medium/high/critical), policy-enforced
(allow/deny/ask), audit-logged, and — for "ask" tools — held pending
until the user types an approval ("yes") or denial ("no") in chat.

Based on the IBM Skills Network lab "MCP Security with Permissions and
Elicitation" (author: Wojciech "Victor" Fulmyk). Faithful to the lab
with one change: the OpenAI client is initialized lazily on first chat
(the lab creates it in __init__, which raises without OPENAI_API_KEY
and blocks credential-free import/testing).

Usage:
    export OPENAI_API_KEY="sk-..."
    python mcp_permission_host_app.py mcp_permission_server.py
    # then open http://127.0.0.1:7864
"""
...
class MCPPermissionHostApp(MCPPermissionClient):
    ...
    def __init__(self, server_script: str):
        ...
        self._llm_client = None  # Lazy init (see module docstring)
    ...
    # ------------------------------------------------------------------
    # Tool schema assembly (real MCP tools + synthetic helpers)
    # ------------------------------------------------------------------

    async def get_available_tools(self):
        """All tools in OpenAI function-calling format with permission info."""
        await self.connect()

        mcp_tools = await self.list_tools()
        openai_tools = []

        # Real MCP tools with permission information
    ...
    async def execute_tool(self, tool_name: str, arguments: dict):
        ...
        # Real MCP tools with permission checking
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                if hasattr(content, "text"):
                    ...
                    if ("Permission required for tool:" in result_text
                            and "Please approve this operation"
                            in result_text):
    ...
    def assess_risk(self, tool_name: str, arguments: dict) -> dict:
        ...
        if risk_level == "low":
            ...
            assessment["description"] = (
                "Critical impact - system-level operation"
            )

        return assessment

    # ------------------------------------------------------------------
    # Chat loop with approval workflow
    # ------------------------------------------------------------------
    ...
    async def chat(self, user_message: str, history: list):
        ...
        # Approval response for a pending operation
        if self.pending_approval and user_message.strip().lower() in [
            "yes", "approve", "ok", "confirm", "y"
        ]:
        ...
        # Denial response for a pending operation
        if self.pending_approval and user_message.strip().lower() in [
            "no", "deny", "cancel", "n"
        ]:
            self.pending_approval = None
        ...
        if assistant_message.tool_calls:
            ...
            for tool_call in assistant_message.tool_calls:
                ...
                self.assess_risk(function_name, function_args)

                # Execute (permission checking happens inside)
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python mcp_permission_host_app.py "
