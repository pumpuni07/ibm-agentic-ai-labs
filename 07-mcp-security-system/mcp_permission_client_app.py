...
Four-tab Gradio interface (Tools, Resources, Prompts, Permissions) over
the permission-aware base client: call tools under policy enforcement
with an Approve & Execute flow, read resources, render prompts,
configure per-tool policies, and inspect the audit log.
...
class MCPPermissionClientApp(MCPPermissionClient):
    ...
    async def gui_call_tool(self, tool_selection: str, arguments_json: str,
                            approved: bool = False):
    ...
    def create_interface(self):
        ...
        with gr.Blocks(title="MCP Permission Client") as interface:
            ...
            with gr.Tabs():
                with gr.Tab("Tools"):
                    gr.Markdown(
                        "### List and Call Tools with Permission Enforcement"
                    )
                    with gr.Row():
                        with gr.Column():
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example: python mcp_permission_client_app.py "
